...
export default function ChatWindow({ user, onLogout }) {
  ...
  useEffect(() => {
    ...
    axios.get(`${BACKEND}/api/admin`).then(res => {
      ...
      // Load messages
      axios.get(`${BACKEND}/api/messages/${user.userId}`).then(r => {
        setMessages(r.data);
      });
  ...
  return (
    <div style={styles.container}>
      ...
      <div style={styles.messages}>
        ...
        {messages.map((msg, i) => (
          <div key={msg._id || i} style={{...styles.msgRow, justifyContent: isMe(msg) ? 'flex-end' : 'flex-start'}}>
            <div style={{...styles.bubble, background: isMe(msg) ? 'linear-gradient(135deg,#667eea,#764ba2)' : '#fff',
                color: isMe(msg) ? '#fff' : '#1a1a2e',
                boxShadow: isMe(msg) ? 'none' : '0 1px 4px rgba(0,0,0,0.1)'}}>
              {msg.fileUrl ? renderFileMessage(msg) : <span>{msg.message}</span>}
              <div style={{...styles.time, color: isMe(msg) ? 'rgba(255,255,255,0.7)' : '#999'}}>
                {new Date(msg.timestamp).toLocaleTimeString([], {hour:'2-digit',minute:'2-digit'})}
                {isMe(msg) && <span style={{marginLeft:4}}>{msg.read ? '✓✓' : '✓'}</span>}
              </div>
            </div>
          </div>
        ))}
...
const styles = {
  container: { display:'flex', flexDirection:'column', height:'100vh', maxWidth:480, margin:'0 auto', background:'#f0f2f5' },
